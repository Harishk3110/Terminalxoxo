...
class ProviderRepository:
        ...
        *,
        ...
        item = first_or_none(
            ...
            select(models.ProviderConnection).where(
                models.ProviderConnection.provider_name == provider_name
            ),
        )
        if item is None:
            item = models.ProviderConnection(
    ...
    def list_connections(self) -> list[models.ProviderConnection]:
        return list(
            self.session.execute(
                select(models.ProviderConnection).order_by(models.ProviderConnection.provider_name)
            ).scalars()
        )
    ...
    def record_health(
        ...
        item = models.ProviderHealthSnapshot(
    ...
    def create(
        ...
        *,
        ...
        existing = first_or_none(
            self.session, select(models.RawObject).where(models.RawObject.object_key == object_key)
        )
        if existing:
            return existing
...
class InstrumentRepository:
    ...
    def upsert_exchange(self, code: str, name: str, country: str, timezone: str) -> models.Exchange:
        item = first_or_none(
            self.session, select(models.Exchange).where(models.Exchange.code == code)
        )
        if item is None:
    ...
    def upsert_currency(self, code: str, name: str) -> models.Currency:
        item = first_or_none(
            self.session, select(models.Currency).where(models.Currency.code == code)
        )
        if item is None:
    ...
    def upsert_instrument(self, **kwargs: Any) -> models.Instrument:
        stmt = select(models.Instrument).where(
            and_(
                ...
                models.Instrument.exchange_id == kwargs.get("exchange_id"),
            )
        )
    ...
    def list(
        ...
        stmt = (
            select(models.Instrument).order_by(models.Instrument.symbol).limit(limit).offset(offset)
        )
        if query:
            ...
            stmt = (
                ...
                .where(
                    or_(
                        models.Instrument.symbol.ilike(pattern),
    ...
    def by_symbol(self, symbol: str) -> models.Instrument | None:
        return first_or_none(
            self.session, select(models.Instrument).where(models.Instrument.symbol == symbol)
        )
...
class MarketRepository:
    ...
    def price_history(
        self, instrument_id: str, interval: str = "1d", limit: int = 3000
    ) -> list[models.PriceBar]:
        stmt = (
            select(models.PriceBar)
            .where(
                and_(
                    models.PriceBar.instrument_id == instrument_id,
                    models.PriceBar.interval == interval,
                )
            )
    ...
    def upsert_fx(
        self, base: str, quote: str, value_date: date, rate: Decimal, provider: str, quality: str
    ) -> None:
        item = first_or_none(
            self.session,
            select(models.FxRate).where(
                and_(
                    models.FxRate.base_currency == base,
                    models.FxRate.quote_currency == quote,
                    models.FxRate.date == value_date,
                    models.FxRate.provider == provider,
                )
            ),
        )
        if item is None:
    ...
    def fx_rate(self, base: str, quote: str, on_date: date | None = None) -> Decimal:
        ...
        stmt = (
            ...
            .where(and_(models.FxRate.base_currency == base, models.FxRate.quote_currency == quote))
...
class MacroRepository:
    ...
    def list_series(self, query: str | None = None) -> list[models.MacroSeries]:
        ...
        if query:
            stmt = (
                ...
                .where(
                    or_(
                        models.MacroSeries.series_id.ilike(f"%{query}%"),
                        models.MacroSeries.title.ilike(f"%{query}%"),
                    )
                )
                .order_by(models.MacroSeries.series_id)
            )
        return list(self.session.execute(stmt).scalars())
    ...
    def get_series(self, series_id: str) -> models.MacroSeries | None:
        return first_or_none(
            self.session,
            select(models.MacroSeries).where(models.MacroSeries.series_id == series_id),
        )

    def observations(self, series_id: str, limit: int = 4000) -> list[models.MacroObservation]:
        stmt = (
            select(models.MacroObservation)
            .where(models.MacroObservation.series_id == series_id)
            .order_by(models.MacroObservation.observation_date.desc())
            .limit(limit)
        )
        return list(reversed(self.session.execute(stmt).scalars().all()))
    ...
    def latest_pair(
        self, series_id: str
    ) -> tuple[models.MacroObservation | None, models.MacroObservation | None]:
        rows = list(
            self.session.execute(
                select(models.MacroObservation)
                .where(models.MacroObservation.series_id == series_id)
...
class PortfolioRepository:
    ...
    def default_portfolio(self) -> models.Portfolio | None:
        return first_or_none(
            self.session, select(models.Portfolio).where(models.Portfolio.is_default.is_(True))
        )
    ...
    def transactions(self, portfolio_id: str) -> list[models.PortfolioTransaction]:
        return list(
            self.session.execute(
                ...
                .order_by(
                    models.PortfolioTransaction.trade_date, models.PortfolioTransaction.created_at
                )
            ).scalars()
        )
    ...
    def positions(self, portfolio_id: str) -> list[models.PortfolioPosition]:
        return list(
            self.session.execute(
                select(models.PortfolioPosition)
                .where(models.PortfolioPosition.portfolio_id == portfolio_id)
                .order_by(models.PortfolioPosition.market_value.desc())
            ).scalars()
        )
    ...
    def cash(self, portfolio_id: str) -> list[models.PortfolioCashBalance]:
        return list(
            self.session.execute(
                select(models.PortfolioCashBalance)
                .where(models.PortfolioCashBalance.portfolio_id == portfolio_id)
                .order_by(models.PortfolioCashBalance.currency)
            ).scalars()
        )
    ...
    def replace_positions(
        self,
        portfolio_id: str,
        positions: Iterable[models.PortfolioPosition],
        cash: Iterable[models.PortfolioCashBalance],
        nav: models.NavSnapshot,
    ) -> None:
        self.session.execute(
        ...
        self.session.execute(
            delete(models.PortfolioCashBalance).where(
                models.PortfolioCashBalance.portfolio_id == portfolio_id
            )
        )
        ...
        existing_nav = first_or_none(
            ...
            select(models.NavSnapshot).where(
                and_(
                    models.NavSnapshot.portfolio_id == nav.portfolio_id,
                    models.NavSnapshot.as_of == nav.as_of,
                )
            ),
        )
    ...
    def latest_nav(self, portfolio_id: str) -> models.NavSnapshot | None:
        return first_or_none(
            ...
            .order_by(models.NavSnapshot.as_of.desc()),
        )
...
class AnalyticsRepository:
    ...
    def upsert_performance(self, portfolio_id: str, **kwargs: Any) -> models.PerformanceSnapshot:
        item = first_or_none(
            ...
            .where(models.PerformanceSnapshot.portfolio_id == portfolio_id)
    ...
    def upsert_risk(self, portfolio_id: str, **kwargs: Any) -> models.RiskSnapshot:
        item = first_or_none(
            ...
            .where(models.RiskSnapshot.portfolio_id == portfolio_id)
            .order_by(models.RiskSnapshot.as_of.desc()),
        )
        if item is None:
    ...
    def list_stress_results(self, portfolio_id: str) -> list[models.StressResult]:
        return list(
            self.session.execute(
                select(models.StressResult)
                .where(models.StressResult.portfolio_id == portfolio_id)
                .order_by(models.StressResult.created_at.desc())
            ).scalars()
        )
    ...
    def latest_hedge(self, portfolio_id: str) -> models.HedgeRecommendation | None:
        return first_or_none(
            self.session,
            select(models.HedgeRecommendation)
            .where(models.HedgeRecommendation.portfolio_id == portfolio_id)
            .order_by(models.HedgeRecommendation.created_at.desc()),
        )
...
class JobRepository:
    ...
    def list(self, status: str | None = None) -> list[models.IngestionJob]:
        stmt = (
            select(models.IngestionJob).order_by(models.IngestionJob.created_at.desc()).limit(200)
        )
        if status:
            stmt = (
                select(models.IngestionJob)
                .where(models.IngestionJob.status == status)
...
class DatasetRepository:
    ...
    def list_datasets(self) -> list[models.Dataset]:
        return list(
            self.session.execute(
                select(models.Dataset).order_by(models.Dataset.created_at.desc())
            ).scalars()
        )
...
class StrategyRepository:
    ...
    def latest_version(self, strategy_id: str) -> models.StrategyVersion | None:
        return first_or_none(
            ...
            .order_by(models.StrategyVersion.version.desc()),
        )

    def backtest_runs(self) -> list[models.BacktestRun]:
        return list(
            self.session.execute(
                select(models.BacktestRun).order_by(models.BacktestRun.created_at.desc())
            ).scalars()
        )
    ...
    def backtest_metrics(self, run_id: str) -> list[models.BacktestMetric]:
        return list(
            self.session.execute(
                select(models.BacktestMetric)
                .where(models.BacktestMetric.backtest_run_id == run_id)
                .order_by(models.BacktestMetric.metric)
            ).scalars()
        )
    ...
    def backtest_curve(self, run_id: str) -> list[models.BacktestEquityCurve]:
        return list(
            self.session.execute(
                select(models.BacktestEquityCurve)
                .where(models.BacktestEquityCurve.backtest_run_id == run_id)
                .order_by(models.BacktestEquityCurve.date)
            ).scalars()
        )
...
class SystemRepository:
    ...
    def counts(self) -> dict[str, int]:
        ...
        return {
            name: int(self.session.execute(select(func.count()).select_from(model)).scalar_one())
