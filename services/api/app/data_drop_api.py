    ...
    return {
        "items": [
            ...
            for r in session.scalars(
                select(models.MappingProfile).order_by(
                    models.MappingProfile.code, models.MappingProfile.version.desc()
                )
            ).all()
        ]
    }
        ...
        "items": [
            {
                ...
                if r.last_seen and now - utc(r.last_seen) < timedelta(seconds=90)
                else "OFFLINE",
                "status": r.status,
            }
            for r in session.scalars(select(models.LocalAgent)).all()
        ]
    }
...
def claim(payload: ClaimRequest, session: Session = SESSION_DEPENDENCY):
    ...
    row = session.scalar(
        select(models.AgentPairing).where(
            models.AgentPairing.code_hash == hashlib.sha256(payload.code.encode()).hexdigest()
        )
    )
    if row is None or row.claimed_at or utc(row.expires_at) <= now:
        raise HTTPException(403, "Pairing code is invalid or expired")
    changed = session.execute(
        update(models.AgentPairing)
        .where(
            models.AgentPairing.id == row.id,
            models.AgentPairing.claimed_at.is_(None),
            models.AgentPairing.expires_at > now,
        )
...
def agent_auth(request: Request, session: Session = SESSION_DEPENDENCY) -> models.LocalAgent:
    ...
    agent = session.scalar(
        select(models.LocalAgent).where(
            models.LocalAgent.token_hash == digest, models.LocalAgent.revoked_at.is_(None)
        )
    )
    if agent is None:
...
async def agent_upload(
    ...
    data = await file.read(25_000_001)
    return checked(
        lambda: DataDropService(session).receive(
            file.filename or "upload.csv", data, agent_id=agent.id
        ),
        session,
    )
