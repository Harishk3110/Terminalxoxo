...
class DemoIngestionService:
    ...
    def seed(self, *, reset: bool = False) -> dict[str, int]:
        if reset:
            raise ValueError(
                "Destructive global reset is disabled. Archive a portfolio and create a new demo instead."
            )
        ...
        self.providers.upsert_connection(
            ...
            provider_type="multi_asset",
            enabled=True,
            configured=True,
            state="CONNECTED",
            capabilities=["market_data", "macro", "fundamentals", "portfolio"],
        )
        self.providers.upsert_connection(
            provider_name="IBKR Paper Agent",
            provider_type="broker",
            enabled=True,
            configured=False,
            state="NOT_CONFIGURED",
            capabilities=["account_summary", "positions", "fills", "commissions", "read_only"],
        )

        raw_payload = self._deterministic_payload()
        raw_bytes = json.dumps(raw_payload, sort_keys=True).encode("utf-8")
        stored = self.storage.put_bytes(
            key="raw/demo/seed.json", data=raw_bytes, content_type="application/json"
        )
        raw_object = self.raw.create(
            provider="DemoProvider",
            dataset="deterministic-seed",
            object_key=stored.object_key,
            content_hash=stored.content_hash,
            content_type="application/json",
            size_bytes=stored.size_bytes,
            source_uri="local-demo",
            correlation_id="seed-demo",
        )

        exchanges = {
            "NASDAQ": self.instruments.upsert_exchange(
                "NASDAQ", "Nasdaq Stock Market", "United States", "America/New_York"
            ),
            "NYSE": self.instruments.upsert_exchange(
                "NYSE", "New York Stock Exchange", "United States", "America/New_York"
            ),
            "ARCA": self.instruments.upsert_exchange(
                "ARCA", "NYSE Arca", "United States", "America/New_York"
            ),
            "SGX": self.instruments.upsert_exchange(
                "SGX", "Singapore Exchange", "Singapore", "Asia/Singapore"
            ),
            "CME": self.instruments.upsert_exchange(
                "CME", "Chicago Mercantile Exchange", "United States", "America/Chicago"
            ),
        }
        for code, name in [
        ...
        for (
            ...
            if (
                not self.session.execute(
                    select(models.ProviderInstrumentMapping).where(
                        and_(
                            models.ProviderInstrumentMapping.provider == "DemoProvider",
                            models.ProviderInstrumentMapping.provider_symbol == symbol,
                        )
                    )
                )
                .scalars()
                ...
                self.session.add(
                    models.ProviderInstrumentMapping(
                        instrument_id=instruments[symbol].id,
                        provider="DemoProvider",
                        provider_symbol=symbol,
                        provider_metadata={"quality": QUALITY_DEMO},
                    )
                )
        ...
        while current <= end:
            if current.weekday() < 5:
                for idx, (symbol, *_rest) in enumerate(instrument_specs):
                    ...
                    drift = Decimal(day_count) * (
                        Decimal("0.006") + Decimal(idx) / Decimal("10000")
                    )
                ...
                self.market.upsert_fx(
                    "SGD", "USD", current, Decimal("1") / usdsgd, "DemoProvider", QUALITY_DEMO
                )
                day_count += 1
    ...
    def _seed_intraday(self, instruments: dict[str, models.Instrument], raw_object_id: str) -> None:
        start = datetime(2026, 6, 8, 13, tzinfo=UTC)
    ...
    def _seed_macro(self, raw_object_id: str) -> None:
        ...
        for index, item in enumerate(watchlist):
            ...
            while current <= date(2026, 8, 31):
                value = (
                    Decimal("1.0")
                    + Decimal(index) / Decimal("2")
                    + Decimal(obs_index % 48) / Decimal("25")
                )
    ...
    def _seed_data_operations(self, raw_object_id: str) -> None:
        dataset = models.Dataset(
            name="demo-equity-prices",
            dataset_type="price_bars",
            source="DemoProvider",
            quality=QUALITY_DEMO,
        )
        fundamentals = models.Dataset(
        ...
        self.session.flush()
        price_version = models.DatasetVersion(
            dataset_id=dataset.id,
            version=1,
            raw_object_id=raw_object_id,
            row_count=52260,
            content_hash="demo-price-bars-v1",
            schema_json={
                "columns": [
                    {"name": "date", "type": "date"},
        ...
        fundamentals_version = models.DatasetVersion(
            dataset_id=fundamentals.id,
            version=1,
            raw_object_id=raw_object_id,
            row_count=80,
            content_hash="demo-fundamentals-v1",
            schema_json={
                "columns": [
            ...
            for column in version.schema_json.get("columns", []):
                self.session.add(
                    models.DatasetColumn(
                        ...
                        mapped_role=column["name"],
                    )
                )
            self.session.add(
                models.DatasetLineage(
                    dataset_version_id=version.id,
                    source_type="raw_object",
                    source_id=raw_object_id,
                    transform="deterministic-demo-normalisation",
                )
            )
        succeeded = models.IngestionJob(
            job_type="demo_market_close",
            provider="DemoProvider",
            parameters={"date": "2026-09-04"},
            status="SUCCEEDED",
            progress=Decimal("1"),
            records_received=52260,
            records_accepted=52260,
            records_rejected=0,
            correlation_id="seed-demo-job-ok",
            worker_id="seed",
            started_at=datetime(2026, 9, 5, 0, 0, tzinfo=UTC),
            finished_at=datetime(2026, 9, 5, 0, 1, tzinfo=UTC),
        )
        failed = models.IngestionJob(
            job_type="fred_refresh_default_watchlist",
            provider="FRED",
            parameters={"reason": "missing credentials"},
            status="FAILED",
            progress=Decimal("1"),
            records_received=0,
            records_accepted=0,
            ...
            error_message="FRED_API_KEY is not configured",
            correlation_id="seed-demo-job-fred",
            worker_id="seed",
            started_at=datetime(2026, 9, 5, 0, 2, tzinfo=UTC),
            finished_at=datetime(2026, 9, 5, 0, 2, tzinfo=UTC),
        )
        ...
        self.session.add_all(
            [
                models.IngestionJobRun(
                    job_id=succeeded.id,
                    state="SUCCEEDED",
                    message="Demo price bars normalised",
                    log_payload={"quality": QUALITY_DEMO},
                ),
                models.IngestionJobRun(
                    job_id=failed.id,
                    state="FAILED",
                    message="FRED credentials unavailable",
                    log_payload={"state": "NOT_CONFIGURED"},
                ),
                models.DataQualityIssue(
                    dataset="FRED default watchlist",
                    provider="FRED",
                    severity="WARN",
                    issue_type="PROVIDER_NOT_CONFIGURED",
                    message="FRED backfill skipped until FRED_API_KEY is configured.",
                ),
                models.QuarantineRecord(
                    dataset_version_id=price_version.id,
                    row_number=0,
                    payload={"symbol": "DEMO", "quality": QUALITY_DEMO},
                    error_message="Demo quarantine example for validation workflow",
                ),
            ]
        )
        for component, state in [
            ...
            self.session.add(
                models.SystemHealthSnapshot(
                    component=component,
                    state=state,
                    details={"quality": QUALITY_DEMO},
                    checked_at=datetime(2026, 9, 5, 0, 5, tzinfo=UTC),
                )
            )
    ...
    def _seed_portfolio(self, instruments: dict[str, models.Instrument]) -> str:
        portfolio = models.Portfolio(
            ...
            base_currency="SGD",
            reference_capital=Decimal("70000"),
            is_default=True,
        )
        ...
        account = models.PortfolioAccount(
            ...
            display_name="KnK SGD 70,000 Reference Account",
            provider="DemoProvider",
        )
        ...
        txns = [
            ...
            (
                ...
                "USD",
                "1.345",
                "Initial quality growth allocation",
            ),
            (
                "BUY",
                "MSFT",
                ...
                "USD",
                "1.344",
                "Software compounder allocation",
            ),
            ...
            (
                ...
                "SGD",
                "1",
                "Singapore home-market exposure",
            ),
        ]
        for txn_type, symbol, trade_date, quantity, price, currency, fx, notes in txns:
            ...
            fee = (
                Decimal("7.50")
                if txn_type == "BUY"
                else (Decimal("35.00") if txn_type == "FEE" else Decimal("0"))
            )
            self.session.add(
    ...
    def _seed_research_quant_alerts(self, instruments: dict[str, models.Instrument]) -> None:
        ...
        for symbol in ["AAPL", "MSFT", "SPY", "D05"]:
            self.session.add(
                models.WatchlistItem(
                    watchlist_id=watchlist.id, instrument_id=instruments[symbol].id
                )
            )
        for title in [
            "Quality compounder thesis",
            "Macro disinflation thesis",
            "Singapore bank resilience thesis",
        ]:
            thesis = models.InvestmentThesis(
                title=title,
                thesis_state="ACTIVE",
                summary=f"{title} uses deterministic demo data only.",
                instrument_id=None,
            )
            ...
            self.session.add(
                models.ThesisSource(
                    thesis_id=thesis.id,
                    source_type="demo-research",
                    citation="KnK deterministic research fixture",
                )
            )
        self.session.add(
            models.ResearchNote(
                instrument_id=instruments["MSFT"].id,
                title="Private demo thesis note",
                body="Private portfolio research note backed by deterministic demo data.",
                visibility="PRIVATE",
            )
        )
        self.session.add(
            models.DecisionJournal(
                decision_type="REFERENCE_PORTFOLIO_SEED",
                rationale="Seeded SGD 70,000 reference portfolio for local deterministic acceptance.",
            )
        )
        strategy_specs = [
        ...
        for name, strategy_type, rules in strategy_specs:
            strategy = models.StrategyDefinition(
                name=name,
                strategy_type=strategy_type,
                description=f"{name} deterministic demo strategy.",
                status="AVAILABLE",
            )
        ...
        factor = models.FactorDefinition(
            name="Demo Momentum 126D",
            formula="close / close_126d - 1",
            description="Deterministic factor fixture.",
        )
        self.session.add(factor)
        self.session.flush()
        factor_run = models.FactorRun(
            factor_id=factor.id,
            status="SUCCEEDED",
        ...
        for index, symbol in enumerate(["AAPL", "MSFT", "SPY", "D05"]):
            self.session.add(
                models.FactorValue(
                    ...
                    instrument_id=instruments[symbol].id,
                    date=date(2026, 9, 4),
                    value=Decimal("0.05") + Decimal(index) / Decimal("100"),
                )
            )
        self.session.add(
            models.Alert(
                severity="WARN",
                title="Demo stale provider check",
                message="FRED is not configured; demo macro data remain active.",
                state="ACTIVE",
            )
        )
        self.session.add(
            models.Alert(
                ...
                message="Reference risk snapshot calculated from deterministic data.",
                state="RESOLVED",
...
class PortfolioService:
    ...
    def default_snapshot(self) -> dict:
        ...
        return {
            ...
            "positions": [
                self._position_payload(position) for position in self.repo.positions(portfolio.id)
            ],
            "cash": [self._cash_payload(item) for item in self.repo.cash(portfolio.id)],
            "transactions": [
                self._transaction_payload(item) for item in self.repo.transactions(portfolio.id)
            ],
        }
    ...
    def add_manual_transaction(self, payload: dict) -> dict:
        ...
        if profile_for(self.session):
            ...
            result = PortfolioLedgerService(self.session).add(
        ...
        if payload["transaction_type"] in {"BUY", "SELL"} and (
            not payload.get("symbol")
            or Decimal(str(payload.get("quantity", 0))) <= 0
            or Decimal(str(payload.get("price", 0))) <= 0
        ):
        ...
        if payload["transaction_type"] == "SELL":
            ...
            if Decimal(str(payload["quantity"])) > balance:
                raise ValueError(
                    "Sale exceeds recorded holdings on the trade date; short selling is not supported"
                )
    ...
    def recalculate(self, portfolio_id: str) -> dict:
        ...
        for txn in self.repo.transactions(portfolio_id):
            ...
            if txn.transaction_type == "DEPOSIT":
                cash[currency] = cash.get(currency, Decimal("0")) + (
                    quantity * price or Decimal(portfolio.reference_capital)
                )
            elif txn.transaction_type == "BUY" and txn.instrument_id:
                state = holdings.setdefault(
                    txn.instrument_id, {"qty": Decimal("0"), "cost": Decimal("0")}
                )
                gross_native = quantity * price
                state["cost"] += gross_native
                state["qty"] += quantity
                cash[portfolio.base_currency] = (
                    cash.get(portfolio.base_currency, Decimal("0")) - gross_native * fx - fee
                )
                ...
                cash[portfolio.base_currency] = (
                    cash.get(portfolio.base_currency, Decimal("0")) + proceeds
                )
            elif txn.transaction_type == "DIVIDEND" and txn.instrument_id:
                cash[portfolio.base_currency] = (
                    cash.get(portfolio.base_currency, Decimal("0")) + quantity * price * fx
                )
            elif txn.transaction_type == "FEE":
                cash[portfolio.base_currency] = (
                    cash.get(portfolio.base_currency, Decimal("0")) - fee
                )
        ...
        cash_base = sum(
            (
                ...
                else amount * self.market.fx_rate(currency, portfolio.base_currency)
        ...
        nav_row = models.NavSnapshot(
            portfolio_id=portfolio_id,
            as_of=datetime(2026, 9, 5, tzinfo=UTC),
            nav=quantize_money(nav),
            cash=quantize_money(cash_base),
            market_value=quantize_money(market_value_total),
            quality=QUALITY_DEMO,
        )
    ...
    def _portfolio_payload(
        self, portfolio: models.Portfolio, nav: models.NavSnapshot | None
    ) -> dict:
        return to_jsonable(
            {
                "id": portfolio.id,
                ...
                "reference_capital": portfolio.reference_capital,
                "nav": nav.nav if nav else None,
                "cash": nav.cash if nav else None,
                "market_value": nav.market_value if nav else None,
                "quality": nav.quality if nav else QUALITY_DEMO,
            }
        )
    ...
    def _transaction_payload(self, txn: models.PortfolioTransaction) -> dict:
        instrument = (
            self.session.get(models.Instrument, txn.instrument_id) if txn.instrument_id else None
        )
        return to_jsonable(
            {
                "id": txn.id,
...
class RiskService:
    ...
    def calculate(self, portfolio_id: str) -> dict:
        ...
        gross = sum(
            (abs(Decimal(item.market_value)) for item in positions), Decimal("0")
        ) / Decimal(nav.nav)
        ...
        snapshot = self.analytics.upsert_risk(
            ...
            max_drawdown=max_drawdown(
                [Decimal(nav.nav) * (Decimal("1") + item) for item in returns]
            ),
    ...
    def _ensure_stress_and_hedge(
        self, portfolio_id: str, nav: Decimal, current_beta: Decimal
    ) -> None:
        scenario = (
            self.session.execute(
                select(models.StressScenario).where(
                    models.StressScenario.name == "Equity -10% / USD -2%"
                )
            )
            .scalars()
            .first()
        ...
        if scenario is None:
            scenario = models.StressScenario(
                name="Equity -10% / USD -2%",
                description="Deterministic demo stress scenario.",
                shocks={"equity": -0.10, "usdsgd": -0.02},
            )
            self.session.add(scenario)
            self.session.flush()
        if (
            not self.session.execute(
                select(models.StressResult).where(
                    and_(
                        models.StressResult.portfolio_id == portfolio_id,
                        models.StressResult.scenario_id == scenario.id,
                    )
                )
            )
            .scalars()
            ...
            self.session.add(
                models.StressResult(
                    ...
                    portfolio_id=portfolio_id,
                    as_of=datetime.now(UTC),
                    pnl_impact=quantize_money(nav * Decimal("-0.075")),
                    nav_after=quantize_money(nav * Decimal("0.925")),
                    contributions=[
                        {"symbol": "AAPL", "pnl": "-1650.00"},
                        {"symbol": "MSFT", "pnl": "-1425.00"},
                    ],
                )
            )
        spy = (
            self.session.execute(select(models.Instrument).where(models.Instrument.symbol == "SPY"))
            .scalars()
            .first()
        )
        ...
        if (
            ...
            and not self.session.execute(
                select(models.HedgeInstrument).where(models.HedgeInstrument.instrument_id == spy.id)
            )
            .scalars()
            .first()
        ):
            self.session.add(
                models.HedgeInstrument(
        ...
        units = hedge_units(
            target_notional, Decimal(quote.price) if quote else Decimal("500"), Decimal("1")
        )
        residual = residual_notional(
            target_notional, units, Decimal(quote.price) if quote else Decimal("500"), Decimal("1")
        )
        recommendation = models.HedgeRecommendation(
            portfolio_id=portfolio_id,
            as_of=datetime.now(UTC),
            target_beta=Decimal("0.80"),
            current_beta=current_beta,
            target_notional=quantize_money(target_notional),
            residual_notional=quantize_money(residual),
            quality=QUALITY_DEMO,
        )
        ...
        if hedge_instrument and quote:
            self.session.add(
                models.HedgeRecommendationItem(
                    recommendation_id=recommendation.id,
                    hedge_instrument_id=hedge_instrument.id,
                    units=units,
                    price=Decimal(quote.price),
                    notional=quantize_money(units * Decimal(quote.price)),
                    instruction="Manual ETF hedge recommendation only. No broker action is available.",
                )
            )
    ...
    def stress(self) -> list[dict]:
        ...
        if profile_for(self.session, portfolio.id):
            ...
            if data["portfolio"]["nav"] is None or any(
                p["beta"] is None for p in data["positions"]
            ):
                return []
            results = []
                ...
                results.append(
                    {
                        **result,
                        "scenario_id": scenario["id"],
                        "pnl_impact": result["loss"],
                        "nav_after": result["post_nav"],
        ...
        return [
            ...
            for item in self.analytics.list_stress_results(portfolio.id)
        ]
    ...
    def hedge(self) -> dict:
        ...
        if profile_for(self.session, portfolio.id):
            from .portfolio_valuation import PortfolioValuationService
            from .price_sources import FxRateResolver, MarketPriceResolver

            data = PortfolioValuationService(self.session).latest(portfolio.id)
            spy = self.session.scalar(
                select(models.Instrument).where(models.Instrument.symbol == "SPY")
            )
            now = datetime.now(UTC)
            price = (
                MarketPriceResolver(self.session, [spy.id]).resolve(spy.id, now) if spy else None
            )
            fx, _ = (
                FxRateResolver(self.session).resolve(spy.currency, portfolio.base_currency, now)
                if spy
                else (None, {})
            )
            ...
            if price is None or fx is None or beta is None or nav is None:
                return {
                    ...
                    "items": [],
                    "state": "UNAVAILABLE",
                    "warnings": ["Complete prices, FX and beta history required"],
                }
            target = Decimal(".8")
            ...
            return to_jsonable(
                {
                    "recommendation": {
                        ...
                        "residual_notional": notional - units * price.value * fx,
                    ...
                    "items": [
                        {
                            ...
                            "price": price.value,
                            "fx_rate": fx,
                            "notional": -units * price.value * fx,
                            "instruction": "Manual review required. Indicative beta reduction only; no order is transmitted.",
                        }
                    ]
                    if units
                    else [],
                    ...
                    + [
                        "Assumes SPY beta one; ignores basis, financing, margin and execution constraints."
                    ],
                }
            )
        ...
        if rec:
            rows = self.session.execute(
                select(models.HedgeRecommendationItem).where(
                    models.HedgeRecommendationItem.recommendation_id == rec.id
                )
            ).scalars()
            for row in rows:
                items.append(
        ...
        return to_jsonable(
            {
                "recommendation": {
                    "id": rec.id,
                    "target_beta": rec.target_beta,
                    "current_beta": rec.current_beta,
                    "target_notional": rec.target_notional,
                    "residual_notional": rec.residual_notional,
                    "quality": rec.quality,
                }
                if rec
                else None,
...
class FredIngestionService:
    def __init__(
        self, session: Session, provider: FredProvider, storage: ObjectStorage | None = None
    ):
        self.session = session
    ...
    async def refresh_series(
        ...
        self.provider.settings = self.provider.settings.model_copy(
            update={"fred_enabled": status.enabled}
        )
        if not self.provider.configured:
            ...
            self.session.add(
                models.DataQualityIssue(
                    ...
                    severity="WARN",
                    issue_type="PROVIDER_NOT_CONFIGURED",
                    message="FRED credentials are required before connected macro ingestion can run.",
                )
            )
            self.session.commit()
        ...
        try:
            ...
            observations_payload = await self.provider.observations(
                series_id, correlation_id, observation_start
            )
            ...
            error = (
                ...
                else ProviderError("FRED response validation failed")
            )
        ...
        observations_raw = self._store_raw(
            series_id, "observations", observations_payload, correlation_id
        )
        ...
        if not metadata.seriess:
            self.session.add(
                models.DataQualityIssue(
                    ...
                    provider="FRED",
                    severity="ERROR",
                    issue_type="SERIES_UNAVAILABLE",
                    message=f"FRED did not return metadata for {series_id}.",
                )
            )
            self.session.commit()
        ...
        self.macro.upsert_series(
            ...
            observation_start=date.fromisoformat(series.observation_start)
            if series.observation_start
            else None,
            observation_end=date.fromisoformat(series.observation_end)
        ...
        for vintage in parsed_vintages.vintage_dates:
            if (
                not self.session.execute(
                    select(models.MacroVintage).where(
                        and_(
                            models.MacroVintage.series_id == series.id,
                            models.MacroVintage.vintage_date == date.fromisoformat(vintage),
                            models.MacroVintage.provider == "FRED",
                        )
                    )
                )
                .scalars()
        ...
        record_result(
            ...
            response=Response(
                ...
                b"",
                "FRED observations",
                observations_payload.get("_knk_latency_ms", 0),
                None,
            ),
            action="sync",
        )
        status.last_data_sync = datetime.now(UTC)
        self.session.commit()
        return {
            "state": "SUCCEEDED",
            "records_received": len(parsed_observations.observations),
            "records_accepted": accepted,
        }

    def _store_raw(
        self, series_id: str, payload_type: str, payload: dict, correlation_id: str
    ) -> models.RawObject:
        ...
        return self.raw.create(
            provider="FRED",
            dataset=f"fred-{payload_type}",
            object_key=stored.object_key,
            content_hash=stored.content_hash,
            content_type="application/json",
            size_bytes=stored.size_bytes,
            source_uri=f"fred/{payload_type}/{series_id}",
...
class DatasetService:
    ...
    def ingest_bytes(
        self, *, filename: str, data: bytes, content_type: str, dataset_type: str = "price_bars"
    ) -> dict:
        ...
        raw = self.raw.create(
            ...
            size_bytes=stored.size_bytes,
            source_uri=filename,
            correlation_id=f"upload-{uuid.uuid4()}",
        )
        rows, columns = infer_tabular(data, suffix)
        upload = self.repo.create_upload(
        ...
        dataset, version = self.repo.create_dataset_version(
            ...
            row_count=len(rows),
            content_hash=stored.content_hash,
            schema_json={"columns": columns, "preview": rows[:20]},
        )
        self.session.commit()
        return {
            "upload_id": upload.id,
    ...
    def list(self) -> list[dict]:
        ...
        for version in self.session.scalars(
            select(models.DatasetVersion).order_by(models.DatasetVersion.version.desc())
        ).all():
            ...
            to_jsonable(
                {
                    ...
                    "latest_version_id": latest[item.id].id if item.id in latest else None,
                    "latest_version": latest[item.id].version if item.id in latest else None,
                }
            )
            for item in self.repo.list_datasets()
        ]
...
def infer_tabular(data: bytes, suffix: str) -> tuple[list[dict], list[dict]]:
    if suffix not in {"csv", "json"}:
        raise ValueError(
            "Only CSV and JSON parsing is enabled in the API process; XLSX/Parquet are routed to worker jobs"
        )
    if suffix == "json":
...
class BacktestService:
    ...
    def run_demo_backtest(self) -> dict:
        strategy = (
            self.session.execute(
                select(models.StrategyDefinition).where(
                    models.StrategyDefinition.strategy_type == "moving_average_crossover"
                )
            )
            .scalars()
            .first()
        ...
        existing = (
            self.session.execute(
                select(models.BacktestRun).where(
                    models.BacktestRun.strategy_version_id == version.id
                )
            )
            .scalars()
            .first()
        ...
        signals = moving_average_signals(
            rows, int(version.rules["fast"]), int(version.rules["slow"])
        )
        ...
        for signal in signals:
            ...
            self.session.add(
                models.BacktestPosition(
                    backtest_run_id=run.id,
                    date=signal.signal_date,
                    quantity=qty,
                    market_value=quantize_money(qty * signal.close),
                )
            )
        ...
        for metric, value in metrics.items():
            self.session.add(
                models.BacktestMetric(backtest_run_id=run.id, metric=metric, value=value)
            )
        self.session.flush()
    ...
    def run_payload(self, run_id: str) -> dict:
        ...
        metrics = {
            metric.metric: metric.value for metric in self.strategies.backtest_metrics(run.id)
        }
        ...
        return to_jsonable(
            {
                ...
                "metrics": metrics,
                "equity_curve": [
                    {"date": item.date, "equity": item.equity, "drawdown": item.drawdown}
                    for item in curve
                ],
            }
        )
...
class ReportService:
    ...
    def portfolio_xlsx(self) -> dict:
        ...
        if portfolio.get("calculation_version"):
            ...
            sheets["Sources"] = [
                ...
                *[
                    [k, portfolio[k]]
                    for k in ("source", "as_of", "quality", "calculation_version", "methodology")
                ],
            ]
            sheets["Warnings"] = [["Warning"], *[[w] for w in portfolio["warnings"]]]
            result = self._write_workbook(
                "portfolio-overview", sheets, quality=portfolio["quality"]
            )
        ...
        sheets = {
            "Overview": [
                ...
                ["Reference Capital", portfolio["portfolio"]["reference_capital"]],
                ["NAV", portfolio["portfolio"]["nav"]],
                ["Quality", portfolio["portfolio"]["quality"]],
            ],
            "Holdings": [
                [
                    "Symbol",
            ...
            "Transactions": [
                ["Date", "Type", "Symbol", "Quantity", "Price", "Currency", "FX", "Fee", "Quality"],
                *[
                    [
                        row["trade_date"],
                        row["type"],
                    ...
                    for row in portfolio["transactions"]
                ],
            ],
            "Cash": [
                ["Currency", "Amount", "As Of", "Quality"],
                *[
                    [row["currency"], row["amount"], row["as_of"], row["quality"]]
                    for row in portfolio["cash"]
                ],
            ],
            "NAV": [
                ["Metric", "Value"],
                ["Cash", portfolio["portfolio"]["cash"]],
                ["Market Value", portfolio["portfolio"]["market_value"]],
                ["NAV", portfolio["portfolio"]["nav"]],
            ],
            "Performance": [
                ["Metric", "Value"],
                *[[key, value] for key, value in performance.items() if key not in {"quality"}],
            ],
            "Exposure": [
                ["Metric", "Value"],
                ["Gross Exposure", risk.get("gross_exposure")],
                ["Net Exposure", risk.get("net_exposure")],
                ["Concentration", risk.get("concentration")],
            ],
            "Risk": [["Metric", "Value"], *[[key, value] for key, value in risk.items()]],
            "Stress": [
                ["Scenario ID", "P&L Impact", "NAV After", "Contributions"],
                *[
                    [
                        row["scenario_id"],
                        row["pnl_impact"],
                        row["nav_after"],
                        json.dumps(row["contributions"], sort_keys=True),
                    ]
                    for row in stress
                ],
            ],
            "Hedge": [
                ["Metric", "Value"],
                ["Current Beta", hedge_recommendation.get("current_beta")],
                ["Target Beta", hedge_recommendation.get("target_beta")],
                ["Target Notional", hedge_recommendation.get("target_notional")],
                ["Residual Notional", hedge_recommendation.get("residual_notional")],
            ],
            "Sources": [
                ["Provider", "Dataset", "Quality"],
                ["DemoProvider", "deterministic-seed", QUALITY_DEMO],
            ],
            "Checks": [
                ["Check", "State"],
                ["Formula injection protection", "PASS"],
                ["Broker execution capability", "ABSENT"],
            ],
        }
    ...
    def risk_xlsx(self) -> dict:
        ...
        sheets = {
            ...
            "Sources": [
                ["Provider", "Dataset", "Quality"],
                ["DemoProvider", "risk-calculation", QUALITY_DEMO],
            ],
            "Checks": [
                ["Check", "State"],
                ["Historical VaR available", "PASS"],
                ["CVaR available", "PASS"],
            ],
        }
    ...
    def backtest_xlsx(self) -> dict:
        ...
        trades = (
            self.session.execute(
                ...
                .where(models.BacktestTrade.backtest_run_id == run_id)
                .order_by(models.BacktestTrade.trade_date)
            )
            .scalars()
            .all()
        )
        positions = (
            self.session.execute(
                select(models.BacktestPosition)
                .where(models.BacktestPosition.backtest_run_id == run_id)
                .order_by(models.BacktestPosition.date)
            )
            .scalars()
            .all()
        )
        ...
        sheets = {
            "Summary": [
                ...
                ["Initial Capital", run_payload["initial_capital"]],
                ["Final Equity", run_payload["final_equity"]],
                *[[key, value] for key, value in run_payload["metrics"].items()],
            ],
            "Parameters": [
                ["Parameter", "Value"],
                *[[key, value] for key, value in (run_payload.get("parameters") or {}).items()],
            ],
            ...
            "Trades": [
                ["Date", "Side", "Quantity", "Price", "Fee", "Slippage"],
                *[
                    [row.trade_date, row.side, row.quantity, row.price, row.fee, row.slippage]
                    for row in trades
                ],
            ],
            "Positions": [
                ["Date", "Quantity", "Market Value"],
                *[[row.date, row.quantity, row.market_value] for row in positions],
            ],
            "Exposure": [["Metric", "Value"], ["Long-only", "true"], ["Max allocation", "95%"]],
            "Fees": [
                ["Metric", "Value"],
                ["Commission per fill", "5.00"],
                ["Slippage model", "5 bps"],
            ],
            "Data Sources": [
                ["Provider", "Dataset", "Quality"],
                ["DemoProvider", "1d price bars", QUALITY_DEMO],
            ],
            "Checks": [
                ["Check", "State"],
                ["No look-ahead", "PASS"],
                ["Generated from persisted run", "PASS"],
            ],
        }
    ...
    def macro_xlsx(self) -> dict:
        ...
        sheets: dict[str, list[list[str | None]]] = {
            ...
            "Sources": [
                ["Provider", "Dataset", "Quality"],
                ["DemoProvider/FRED", "macro-series", "Mixed source capable"],
            ],
            "Checks": [
                ["Check", "State"],
                ["Missing values preserved", "PASS"],
                ["Raw object recorded", "PASS"],
            ],
        }
    ...
    def get_report(self, report_id: str) -> dict | None:
        ...
        try:
            ...
            pass
        root = (
            self.storage.local_path(f"reports/{report_id}")
            if hasattr(self.storage, "local_path")
            else None
        )
    ...
    def _write_workbook(
        self, report_slug: str, sheets: dict[str, list[list[Any]]], *, quality: str = "UNVERIFIED"
    ) -> dict:
        import xlsxwriter
        ...
        for sheet_name, rows in sheets.items():
            ...
            for row_index, row in enumerate(rows):
                for column_index, value in enumerate(row):
                    ...
                    self._write_cell(
                        sheet, row_index, column_index, value, currency, pct, cell_format
                    )
        ...
        self.storage.put_bytes(
            key=f"reports/{report_id}/manifest.json",
