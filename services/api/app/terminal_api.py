...
from .terminal_analytics import (
    FUNCTIONS,
    VERSION,
    factor_analysis,
    fundamentals,
...
def workspaces(session: Session = SESSION_DEPENDENCY):
    rows = session.scalars(select(models.Workspace).order_by(models.Workspace.created_at)).all()
    if not rows:
        seeds = [
            ("KnK Portfolio", "/overview"),
            ("Global Macro", "/macro"),
            ("US Equities", "/security-master"),
            ("Options", "/functions/opt"),
            ("Earnings", "/functions/earn"),
            ("Quant Lab", "/quant"),
            ("Research", "/research"),
            ("Risk Monitor", "/stress-tests"),
            ("Data Operations", "/data-jobs"),
        ]
        for name, route in seeds:
            ...
            session.add(
                models.WorkspaceState(
                    workspace_id=row.id,
                    configuration={
                        "tabs": [{"id": str(uuid.uuid4()), "route": route, "title": name}],
                        "securities": ["AAPL"],
                        "inspector": True,
                        "rail": True,
                    },
...
def create_workspace(payload: WorkspaceRequest, session: Session = SESSION_DEPENDENCY):
...
def save_workspace(
    workspace_id: str, payload: WorkspaceRequest, session: Session = SESSION_DEPENDENCY
):
    ...
    state = session.scalar(
        select(models.WorkspaceState).where(models.WorkspaceState.workspace_id == row.id)
    )
    if not state:
...
def delete_workspace(workspace_id: str, session: Session = SESSION_DEPENDENCY):
...
def bootstrap(session: Session = SESSION_DEPENDENCY):
    ...
    return {
        ...
        "workspaces": workspaces(session)["items"],
...
@router.get("/quotes")
def quote_list(session: Session = SESSION_DEPENDENCY):
    return {"items": quotes(session)}


@router.get("/prices/{key}")
def prices(key: str, limit: int = 2600, session: Session = SESSION_DEPENDENCY):
    try:
...
def terminal_portfolio(session: Session = SESSION_DEPENDENCY):
    return portfolio_analytics(session)


@router.get("/fundamentals/{key}")
def financials(key: str, session: Session = SESSION_DEPENDENCY):
    try:
...
def launch_worker(run_id):
    ...
    with (logs / f"analysis-{run_id}.log").open("wb") as stream:
        subprocess.Popen(
            [sys.executable, "-m", "app.terminal_worker", run_id],
            env=env,
            stdout=stream,
            stderr=stream,
...
def start_run(payload: RunRequest, request: Request, session: Session = SESSION_DEPENDENCY):
    ...
    if payload.kind == "backtest" and params.get("dataset_id"):
        version = (
            session.get(models.DatasetVersion, params["dataset_version_id"])
            if params.get("dataset_version_id")
            else session.scalar(
                select(models.DatasetVersion)
                .where(models.DatasetVersion.dataset_id == params["dataset_id"])
        ...
        params["mapping"] = (version.schema_json or {}).get(
            "mapping", {"date": "date", "close": "close"}
        )
        ...
        try:
            if payload.kind == "model":
                ...
                source = ResearchInput(
                    **{key: params[key] for key in ResearchInput.model_fields if params.get(key)}
                )
    ...
    run = models.AnalysisRun(
        ...
        status="QUEUED",
        history=[
            {"state": "QUEUED", "at": now, "actor": actor, "message": "Immutable inputs stored"}
        ],
    )
    session.add(run)
    session.flush()
...
def runs(kind: str | None = None, session: Session = SESSION_DEPENDENCY):
...
def run_detail(run_id: str, session: Session = SESSION_DEPENDENCY):
...
def model_artifact(run_id: str, session: Session = SESSION_DEPENDENCY):
    run = session.get(models.AnalysisRun, run_id)
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="model-{run.id}.joblib"',
...
def export_run(run_id: str, session: Session = SESSION_DEPENDENCY):
    ...
    sheets = {
        "Run": [
            ...
            ["Data as of", result.get("as_of")],
            ["Quality", result.get("quality", "UNVERIFIED")],
        ],
        "Results": [columns]
        + [
            [
        ...
        + [[k, json.dumps(v)] for k, v in run["parameters"].items()],
        "Warnings": [[w] for w in result.get("warnings", [])],
    }
    if run["kind"] == "stress":
        sheets["Summary"] = [
            ["Pre NAV", "Estimated P&L", "Post NAV"],
            [result["pre_nav"], result["loss"], ReportFormula("=A2+B2", result["post_nav"])],
        ]
    return ReportService(session)._write_workbook(
        "terminal-run", sheets, quality=result.get("quality", "UNVERIFIED")
    )
...
async def preview_upload(
    ...
    stored = ObjectStorage().put_bytes(
        key=key, data=data, content_type="text/csv" if suffix == "csv" else "application/json"
    )
    ...
    return {
        ...
        "preview": rows[:30],
        "row_count": len(rows),
        "missing": sum(v in (None, "") for row in rows for v in row.values()),
        "duplicates": len(rows) - len({json.dumps(r, sort_keys=True) for r in rows}),
        "source": "USER_UPLOAD",
        "quality": "UNVERIFIED",
        "as_of": upload.created_at.isoformat(),
    }
...
def validate_upload(payload: ImportRequest, session: Session = SESSION_DEPENDENCY):
    return validated_upload(payload, session)[2]


@router.post("/uploads/import")
def import_upload(payload: ImportRequest, session: Session = SESSION_DEPENDENCY):
    upload, raw, result = validated_upload(payload, session)
    if not result["valid"]:
        raise HTTPException(422, result["errors"])
    data = DatasetService(session).ingest_bytes(
        filename=f"{payload.name}.{upload.content_type}", data=raw, content_type=upload.content_type
    )
...
def dataset_detail(dataset_id: str, session: Session = SESSION_DEPENDENCY):
        ...
        if schema.get("curated_key"):
            ...
            schema = {
                **schema,
                ...
                "preview": preview,
                "columns": [{"name": k} for k in preview[0]] if preview else [],
            }
        lineage = session.scalars(
            select(models.DatasetLineage).where(
                models.DatasetLineage.dataset_version_id == version.id
            )
        ).all()
        linked = [
            run_payload(run)
            for run in session.scalars(
                select(models.AnalysisRun).where(models.AnalysisRun.kind == "backtest")
            ).all()
            if run.parameters.get("dataset_version_id") == version.id
        ]
        result.append(
            {
                "id": version.id,
...
@router.get("/research")
def research(session: Session = SESSION_DEPENDENCY):
    return {
        "items": [
            {
                "id": r.id,
                ...
                "source": "KnK research records",
                "as_of": r.updated_at.isoformat(),
            }
            for r in session.scalars(
                select(models.ResearchNote).order_by(models.ResearchNote.updated_at.desc())
            ).all()
        ]
    }
...
def save_research(payload: ResearchRequest, session: Session = SESSION_DEPENDENCY):
...
def terminal_health(session: Session = SESSION_DEPENDENCY):
    ...
    rows = [
        {
            ...
            "latency_ms": measured,
            "as_of": now,
            "detail": "Request handled",
        },
        {
            "service": "SQLite" if settings.database_url.startswith("sqlite") else "PostgreSQL",
            "state": "HEALTHY",
            "latency_ms": measured,
            "as_of": now,
        ...
        rows.append(
            {
                "service": "Redis",
                "state": "HEALTHY",
                "latency_ms": round((time.perf_counter() - before) * 1000, 1),
                "as_of": now,
        ...
        rows.append(
            {
                ...
                "state": "HEALTHY",
                "as_of": now,
                "detail": "Write/read probe succeeded",
            }
        )
    except Exception:
        rows.append(
            {
                ...
                "state": "FAILED",
                "as_of": now,
                "detail": "Write/read probe failed",
            }
        )
    active = session.scalars(
        select(models.AnalysisRun).where(models.AnalysisRun.status.in_(["QUEUED", "RUNNING"]))
    ).all()
    ...
    agent = session.scalar(
        select(models.LocalAgent)
        .where(models.LocalAgent.revoked_at.is_(None))
        .order_by(models.LocalAgent.last_seen.desc())
        .limit(1)
    )
    online = (
        agent
        and agent.last_seen
        and (datetime.now(UTC) - utc(agent.last_seen)).total_seconds() < 90
    )
    rows.append(
        {
            "service": "Local data agent",
            "state": "ONLINE" if online else "OFFLINE",
            "as_of": agent.last_seen.isoformat() if agent and agent.last_seen else None,
            "detail": "Scoped outbound heartbeat" if online else "No current agent heartbeat",
        }
    )
    ...
    nav = (
        session.scalar(
            ...
            .where(models.PortfolioValuationRun.portfolio_id == profile.portfolio_id)
    ...
    rows.append(
        {
            "service": "Portfolio NAV",
            "state": nav.status if nav else "NOT_CALCULATED",
            "as_of": nav.payload.get("as_of") if nav else None,
            "detail": f"{nav.payload.get('quality')}; calculated {nav.created_at.isoformat()}"
            if nav
            else "No valuation run",
        }
    )
    rows.append(
        {
            ...
            if "STALE" in str(nav.payload.get("quality"))
            else nav.payload.get("quality", "UNAVAILABLE"),
            "as_of": nav.payload.get("as_of") if nav else None,
            "detail": "NAV input quality; each provider and instrument retains separate timestamps",
        }
    )
    ...
    for key, name in NAMES.items():
        ...
        observed_at = (
            ...
            if provider["connection_state"] not in ("CONNECTED", "DEMO")
            else provider["last_success"]
        )
        state = provider["connection_state"]
        if state == "CONNECTED" and (
            not observed_at or (datetime.now(UTC) - utc(observed_at)).total_seconds() > 300
        ):
        ...
        rows.append(
            {
                ...
                "detail": f"Last observed connection: {provider['connection_state']}; data sync: {provider['data_freshness']}",
            }
        )
    from .broker_api import current_snapshot

    broker, broker_active = (
        current_snapshot(session, profile.portfolio_id) if profile else (None, False)
    )
    rows.append(
        {
            "service": "IBKR paper agent",
            "state": "READ_ONLY" if broker_active else "OFFLINE",
            "as_of": broker.as_of.isoformat() if broker else None,
            "detail": "Observed paired paper snapshot" if broker else "No broker snapshot received",
        }
    )
        ...
        rows.append(
            {
                ...
                "state": "NOT_VERIFIED",
                "as_of": now,
                "detail": "No current heartbeat or configured probe",
            }
        )
    try:
        commit = (
...
def terminal_report(kind: str, session: Session = SESSION_DEPENDENCY):
    ...
    if kind == "backtest":
        run = session.scalar(
            ...
            .where(models.AnalysisRun.kind == "backtest", models.AnalysisRun.status == "SUCCEEDED")
    ...
    sheets = {
        "Sources": [
            ["Source", "Data as of", "Quality"],
            [data["source"], data["as_of"], data["quality"]],
        ],
        "Positions": [
            ["Symbol", "Quantity", "Price", "FX", "Contract multiplier", "Calculated value SGD"]
        ]
        + [
            [
                r["symbol"],
                ...
                r["fx_rate"],
                float(r.get("contract_multiplier", 1)),
                ReportFormula(
                    f"=B{i + 2}*C{i + 2}*D{i + 2}*E{i + 2}",
                    float(r["quantity"])
                    * float(r["market_price"])
                    * r["fx_rate"]
                    * float(r.get("contract_multiplier", 1)),
        ...
        "Risk": [["Metric", "Value"]] + [[k, v] for k, v in data["risk"].items()],
        "Performance": [["Metric", "Value"]] + [[k, v] for k, v in data["performance"].items()],
        "Warnings": [[w] for w in data["warnings"]],
    }
...
def auth_session(request: Request, session: Session = SESSION_DEPENDENCY):
    ...
    return {
        "authenticated": False,
        "setup_required": get_settings().knk_env == "local-demo"
        and session.scalar(select(models.User.id).limit(1)) is None,
    }


@router.post("/auth/logout")
def logout(request: Request, response: Response, session: Session = SESSION_DEPENDENCY):
    from .auth_sessions import find_session
...
def logout_all(request: Request, response: Response, session: Session = SESSION_DEPENDENCY):
    from .portfolio_api import identity
    from .portfolio_operations import audit
    ...
    rows = session.scalars(
        select(models.UserSession).where(
            models.UserSession.user_id == actor, models.UserSession.revoked_at.is_(None)
        )
    ).all()
    for row in rows:
        row.revoked_at = datetime.now(UTC)
