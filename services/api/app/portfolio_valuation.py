...
from .portfolio_engine import ONE, ZERO, Entry, LedgerState, daily_performance, money, nav_total
...
def load_entries(session: Session, portfolio_id: str) -> tuple[list[Entry], list[dict[str, Any]]]:
    rows = session.execute(
        select(models.PortfolioTransaction, models.TransactionDetail)
        .outerjoin(
            models.TransactionDetail,
            models.TransactionDetail.transaction_id == models.PortfolioTransaction.id,
        )
        .where(models.PortfolioTransaction.portfolio_id == portfolio_id)
        .order_by(
            models.PortfolioTransaction.trade_date,
            models.PortfolioTransaction.created_at,
            models.PortfolioTransaction.id,
        )
    ).all()
    entries, payloads = [], []
    creators: dict[str, str | None] = {}
    creation_logs = session.execute(
        select(models.AuditLog.resource_id, models.AuditLog.actor_user_id)
        .join(
            models.PortfolioTransaction,
            models.PortfolioTransaction.id == models.AuditLog.resource_id,
        )
        .where(
    ...
    for txn, detail in rows:
        ...
        entries.append(
            Entry(
                ...
                price=txn.price,
                fx=txn.fx_rate_to_base,
                amount=amount,
                fee=txn.fee,
                commission=detail.commission if detail else ZERO,
                tax=detail.tax if detail else ZERO,
                instrument_id=txn.instrument_id,
                multiplier=detail.contract_multiplier if detail else ONE,
            ...
            jsonable(
                {
                    ...
                    "base_value": amount * txn.fx_rate_to_base,
                    "fee": txn.fee,
                    "commission": detail.commission if detail else ZERO,
                    "tax": detail.tax if detail else ZERO,
                    "source": txn.source,
...
class PortfolioValuationService:
        ...
        *,
        ...
        if not force:
            cached = self.session.scalar(
                select(models.PortfolioValuationRun)
                .where(
                    models.PortfolioValuationRun.portfolio_id == portfolio.id,
                    models.PortfolioValuationRun.fingerprint == fingerprint,
                )
                .order_by(models.PortfolioValuationRun.created_at.desc())
                .limit(1)
            )
            if cached:
        ...
        run = models.PortfolioValuationRun(
            ...
            nav=Decimal(payload["portfolio"]["nav"])
            if payload["portfolio"]["nav"] is not None
            else None,
            payload={},
        )
        ...
        if run.nav is not None:
            ...
            self.session.execute(
                delete(models.PortfolioPosition).where(
                    models.PortfolioPosition.portfolio_id == portfolio.id
                )
            )
            self.session.execute(
                delete(models.PortfolioCashBalance).where(
                    models.PortfolioCashBalance.portfolio_id == portfolio.id
                )
            )
            for row in payload["positions"]:
                self.session.add(
            ...
            self.session.execute(
                delete(models.DailyReturn).where(models.DailyReturn.portfolio_id == portfolio.id)
            )
                ...
                if row["return"] is not None:
                    self.session.add(
                        models.DailyReturn(
                            ...
                            return_value=Decimal(str(row["return"])),
                        )
                    )
        if commit:
            self.session.commit()
    ...
    def calculate(self, key=None, end=None):
        ...
        benchmark = next(
            (
                r
                for r in instruments.values()
                if r.symbol == profile.configuration.get("benchmark", "SPY")
            ),
            None,
        )
        ids = {r.instrument_id for r in entries if r.instrument_id}
        ids.update(
            r.metadata["child_instrument_id"]
            for r in entries
            if r.metadata.get("child_instrument_id")
        )
        if benchmark:
        ...
        if profile.configuration.get("price_mode") == "DEMO_ONLY":
            ...
            for key in list(prices.grouped):
                prices.grouped[key] = {
                    group: value
                    for group, value in prices.grouped[key].items()
                    if group[0] == "DEMO"
                }
            for pair in list(fx.groups):
                fx.groups[pair] = {
                    group: value for group, value in fx.groups[pair].items() if group[0] == "DEMO"
                }
        ...
        for day in days:
            ...
            daily_attribution = PositionPnlSession(
                {key: lot.quantity for key, lot in state.lots.items()},
                previous_position_values,
            )
            ...
            for currency, cash_balance in state.cash_service.balances(day).items():
                ...
                if (
                    amount == 0
                    and cash_balance.settled == 0
                    and cash_balance.receivable == 0
                    and cash_balance.payable == 0
                ):
                    continue
                ...
                cash_rows.append(
                    {
                        "currency": currency,
                        "amount": amount,
                        "settled": cash_balance.settled,
                        "receivable": cash_balance.receivable,
                        "payable": cash_balance.payable,
                        "available": cash_balance.available,
                        "base_value": money(value) if value is not None else None,
                        "base_value_exact": value,
                        "as_of": provenance["as_of"],
                        "quality": provenance["data_state"],
                        "fx_rate": rate,
                        "fx_provenance": provenance,
            ...
            for instrument_id, lot in state.lots.items():
                ...
                measurement = PortfolioPositionService.measure(
                    lot, price.value if price else None, rate
                )
                ...
                balance_exposure_rows.append(
                    {
                        ...
                        "fx_rate": rate,
                        "fx_provenance": adjustment_provenance,
                    }
                )
                if value is None:
            ...
            totals = nav_total(
                settled_cash,
                list(position_values.values()),
                dict(balances),
                cash_components=settled_cash_components,
            )
            nav = None if missing else totals["nav"]
            daily_pnl, daily_return = (
                (None, None)
                if nav is None or previous_nav is None
                else daily_performance(previous_nav, nav, flows)
            )
            ...
            benchmark_return = (
                (
                    benchmark_base / previous_benchmark - 1
            ...
            benchmark_equity = (
                (benchmark_equity + flows) * (1 + benchmark_return)
                if benchmark_equity is not None and benchmark_return is not None
                else None
            )
            previous_benchmark = benchmark_base
            marked_values = {
                key: position_values.get(key) for key, lot in state.lots.items() if lot.quantity
            }
            daily_details = daily_attribution.finish(
                marked_values, state.cash_service.movements[movement_start:]
            )
            daily_contributions = {key: detail.pnl for key, detail in daily_details.items()}
            for row in position_rows:
                row["weight"] = (
                    position_values[row["instrument_id"]] / nav
                    if nav and row["market_value"] is not None
                    else ZERO
                )
                row["daily_pnl"] = (
                    number(daily_contributions.get(row["instrument_id"], ZERO))
            ...
            curve.append(
                {
                    "date": day.isoformat(),
                    "equity": number(money(nav)) if nav is not None else None,
                    "opening_nav": number(money(previous_nav))
                    if previous_nav is not None
                    else None,
                    "daily_pnl": number(money(daily_pnl)) if daily_pnl is not None else None,
                    "external_flow": number(money(flows)),
                    "return": number(daily_return),
                    "nav_exact": nav,
                    "opening_nav_exact": previous_nav,
                    "pnl_exact": daily_pnl,
                    "external_flow_exact": flows,
                    "net_return_exact": daily_return,
                    "fee_expense_exact": state.fees
                    - fees_before
                    + balances["accrued_fees"]
                    - previous_accrued_fees
                    if not missing
                    else None,
                    "benchmark": number(money(benchmark_equity))
                    if benchmark_equity is not None
                    else None,
                    "benchmark_return": number(benchmark_return),
                    "benchmark_return_exact": benchmark_return,
                    "drawdown": dd,
                    "return_index": number(return_index),
                    "quality": "UNAVAILABLE"
                    if missing
                    else ("STALE" if any(p.get("stale") for p in sources) else "CALCULATED"),
                }
            )
            previous_nav = nav
            previous_accrued_fees = balances["accrued_fees"]
            previous_position_values = marked_values
            last_positions, last_cash, last_daily_contributions = (
                position_rows,
                cash_rows,
                daily_contributions,
            )
        ...
        position_weights = position_exposures(
            [
                ExposurePosition(
                    row["instrument_id"],
                    row["sector"],
                    position_values.get(row["instrument_id"]),
        ...
        risk.update(
            {
                ...
                if nav and all(r["weight"] is not None for r in exposure["sector"])
                else None,
                "max_currency_weight": number(
                    max((abs(r["weight"]) for r in exposure["currency"]), default=ZERO)
                )
                if nav and all(r["weight"] is not None for r in exposure["currency"])
                else None,
                "max_country_weight": number(
                    max((abs(r["weight"]) for r in exposure["country"]), default=ZERO)
                )
                if nav and all(r["weight"] is not None for r in exposure["country"])
                else None,
                "max_industry_weight": number(
                    max((abs(r["weight"]) for r in exposure["industry"]), default=ZERO)
                )
                if nav and all(r["weight"] is not None for r in exposure["industry"])
                else None,
        ...
        limits = self.session.execute(
            ...
            .where(
                models.RiskPolicy.portfolio_id == portfolio.id, models.RiskPolicy.enabled.is_(True)
            )
        ).all()
        breaches = []
        for limit, _ in limits:
            ...
            if actual is not None and (
                actual > float(limit.threshold)
                if limit.direction == "MAX"
                else actual < float(limit.threshold)
            ):
                breaches.append(
                    {
                        "limit_id": limit.id,
        ...
        if any(p.get("stale") for p in selected_sources):
            warnings.append(
                "CALCULATED WITH STALE DATA: selected prices/FX are retained; no demo substitution"
            )
            warnings.append(
                "Risk estimates include carried-forward stale observations and may understate variability"
            )
        if any(p.get("conflict") for p in selected_sources):
            warnings.append(
                "Material same-date source differences exceed 1%; review price provenance"
            )
        ...
        warnings.append(
            "Risk uses current security weights; cash FX and liability sensitivities are excluded. Historical valuations use currently accepted source versions, not point-in-time research snapshots."
        )
        ...
        market_sources = sorted(
            {p["source"] for p in selected_sources if p.get("source") != "IDENTITY"}
        )
        source_name = "INTERNAL LEDGER" + (
        ...
        investment = sum((lot.realised for lot in state.lots.values()), ZERO) + sum(
            (position_values.get(key, ZERO) - lot.cost_base for key, lot in state.lots.items()),
            ZERO,
        )
        ...
        for key, lot in state.lots.items():
            ...
            unrealised = (
                position_values.get(key, ZERO) - lot.cost_base
                if key in position_values or lot.quantity == 0
                else None
            )
            contribution = (
                lot.realised + unrealised + lot.income - lot.expensed_charges
            ...
            attribution.append(
                {
                    ...
                    "sector": item.sector,
                    "currency": item.currency,
                    "realised": money(lot.realised),
                    "unrealised": money(unrealised) if unrealised is not None else None,
                    "income": money(lot.income),
                    "fees": money(lot.charges),
                    "daily_pnl": money(daily_value)
                    if nav is not None and daily_value is not None
                    else None,
                    "daily_pnl_details": daily_details[key].payload()
                    if key in daily_details
                    else None,
                    "total_pnl": money(contribution) if contribution is not None else None,
                    "contribution": number(contribution / portfolio.reference_capital)
        ...
        metric_metadata = {
            key: {
                "value": value,
                "portfolio": profile.code,
                "base_currency": portfolio.base_currency,
                "start": start.isoformat(),
                "end": end.isoformat(),
                "benchmark": profile.configuration.get("benchmark", "SPY"),
                "methodology": METHOD,
                "source": source_name,
                "calculated_at": now.isoformat(),
                "quality": quality,
                **states.get(
                    key, {"state": "AVAILABLE" if value is not None else "INSUFFICIENT_DATA"}
                ),
            }
            for key, value in {**performance, **risk}.items()
        }
        balance_sheet_difference = (
            money(final_totals["gross_asset_value"] - final_totals["liabilities"] - nav, 8)
            if nav is not None
            else None
        )
        ...
        p = {
            ...
            "opening_capital": portfolio.reference_capital,
            **(
                {k: money(v) if isinstance(v, Decimal) else v for k, v in final_totals.items()}
                if nav is not None
                else dict.fromkeys(final_totals)
            ),
            "nav": money(nav) if nav is not None else None,
            "total_pnl": money(total_pnl) if total_pnl is not None else None,
            "cash": money(total_cash)
            if all(c["base_value"] is not None for c in last_cash)
            else None,
            "market_value": money(final_totals["market_value"])
            if all(p["market_value"] is not None for p in last_positions)
            else None,
            "gross_asset_value": money(final_totals["gross_asset_value"])
            if nav is not None
            else None,
            "total_return": number(total_pnl / portfolio.reference_capital)
            ...
            "daily_pnl": curve[-1]["daily_pnl"],
            "opening_nav": curve[-1]["opening_nav"],
            "quality": quality,
            "as_of": as_of,
            "calculated_at": now.isoformat(),
            "view": "INTERNAL LEDGER",
        ...
        postings = [
            posting for entry in entries for posting in transaction_postings(entry, state.policy)
        ]
        marks = {
            currency: CurrencyMark(*fx.resolve(currency, portfolio.base_currency, at))
        ...
        postings.extend(
            outstanding_postings(
                ...
                [
                    BalanceAdjustment(
                        row.id, row.effective_date, row.bucket, row.currency, row.amount, row.reason
                    )
        ...
        return jsonable(
            {
                ...
                "balance_sheet": {
                    ...
                    if balance_sheet_difference is None
                    else "BREAK"
                    if balance_sheet_difference
                    else "BALANCED",
                ...
                "cost_basis": {
                    "method": state.policy.method.value,
                    "capitalized_charges": state.capitalized_charges,
                    "expensed_fees": state.expensed_fees,
                },
                "lots": [
                    asdict(lot) for lots in state.cost_basis.open_lots.values() for lot in lots
                ],
                "lot_matches": [asdict(match) for match in state.cost_basis.matches],
                "accounting": {
                    "items": [row.payload() for row in postings],
                    "totals": posting_totals,
                    "reconciliation": {
                        "state": "BREAK" if any(posting_differences.values()) else "BALANCED",
                ...
                "freshness": marked_exposure.freshness(),
                "reconciliation": {
                    "opening_capital": portfolio.reference_capital,
                    "net_external_flows": money(state.external_flows),
                    "additional_capital_flows": money(
                        state.external_flows - portfolio.reference_capital
                    ),
                    "investment_pnl": money(investment) if nav is not None else None,
                    "income": money(state.income),
                    "fees": money(state.fees),
                    "taxes": money(state.taxes),
                    "cash_fx_pnl": money(cash_fx) if nav is not None else None,
                    "adjustments": money(state.adjustments + balance_effect),
                    "expected_nav": money(expected) if nav is not None else None,
