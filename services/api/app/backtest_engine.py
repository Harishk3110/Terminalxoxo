...
def signals_for(frames: dict[str, pd.DataFrame], settings: BacktestSettings):
...
def risk_parity(covariance: np.ndarray) -> np.ndarray:
    count = len(covariance)
    if count == 1:
        return np.ones(1)
    if not np.isfinite(covariance).all() or np.linalg.eigvalsh(covariance).min() < -1e-10:
        raise ValueError("Risk parity needs a finite positive-semidefinite covariance matrix")

    def objective(weights):
        contributions = weights * (covariance @ weights)
        total = contributions.sum()
        return np.sum((contributions / total - 1 / count) ** 2) if total > 0 else 1e6
...
def target_weights(signal, volatility, trailing, settings):
    valid = signal.dropna()
    ...
    return weights.clip(-settings.position_limit, settings.position_limit).to_dict()
...
def simulate(
    ...
    curve, fills, closed, rejected, snapshots, positions = [], [], [], [], [], []
    benchmark = (
    ...
    class ResearchStrategy(bt.Strategy):
        ...
        def next(self):
            ...
            weights = target_weights(
                signal.loc[day], vol.loc[day], returns.loc[:day].tail(settings.slow), settings
            )
    ...
    metrics = {
        ...
        "cagr": float((equity.iloc[-1] / settings.capital) ** (1 / years) - 1)
        if years >= 1
        else None,
    ...
    metrics["alpha"] = alpha["annualised_alpha"]
    monthly = [
        {"period": str(period), "return": float(np.prod(1 + group) - 1)}
        for period, group in realised.groupby(realised.index.to_period("M"))
    ]
    annual = [
        {"period": str(period), "return": float(np.prod(1 + group) - 1)}
        for period, group in realised.groupby(realised.index.to_period("Y"))
    ]
    return {
        "equity_curve": curve,
        "metrics": metrics,
        "daily_returns": [
            ...
            for day, value in realised.items()
        ],
