...
from app.terminal_worker import backtest_result, execute_run
...
def test_registry_is_unique_and_bootstrap_matches() -> None:
    ...
    assert all(
        q["source"] and q["as_of"] and q["market_state"] == "DEMO" for q in payload["quotes"]
    )
...
def test_workspace_round_trip() -> None:
    ...
    created = client.post(
        "/api/v1/workspaces",
        json={"name": "Test " + str(uuid.uuid4()), "configuration": configuration},
    ).json()
    changed = client.post(
        f"/api/v1/workspaces/{created['id']}",
        json={"name": "Renamed test", "configuration": configuration},
    )
    assert changed.status_code == 200
    saved = next(
        w for w in client.get("/api/v1/workspaces").json()["items"] if w["id"] == created["id"]
    )
...
def test_risk_and_stress_are_recomputed_and_reconcile() -> None:
    with SessionLocal() as session:
        ...
        assert b["loss"] == pytest.approx(2 * a["loss"], abs=0.05)
        assert fx["loss"] > 0
...
def test_run_lifecycle_and_immutable_inputs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    queued = client.post(
        ...
        json={"kind": "stress", "name": "Unit stress", "parameters": {"equity_shock": -10}},
    )
...
def test_upload_validation_import_and_backtest(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    csv = "day,px,open,high,low\n" + "\n".join(
        f"{d.date()},{100 + 0.1 * i + 10 * math.sin(i / 8)},{100 + 0.1 * i + 10 * math.sin(i / 8)},{102 + 0.1 * i + 10 * math.sin(i / 8)},{98 + 0.1 * i + 10 * math.sin(i / 8)}"
        for i, d in enumerate(dates)
    )
    preview = client.post(
        "/api/v1/uploads/preview", files={"file": ("fixture.csv", csv, "text/csv")}
    )
    ...
    payload = {
        ...
        "name": "Unit fixture",
        "mapping": {"date": "day", "close": "px", "open": "open", "high": "high", "low": "low"},
        "licence": "Self-created test data",
    }
...
@pytest.mark.parametrize(
    "filename,body", [("bad.json", "[1,2]"), ("bad.json", "false"), ("bad.xlsx", "not a workbook")]
)
def test_malformed_files_return_validation_error(filename, body):
    assert (
        client.post("/api/v1/uploads/preview", files={"file": (filename, body)}).status_code == 422
    )
...
def test_concurrent_first_load_of_synthetic_financials() -> None:
    ...
    with SessionLocal() as session:
        ...
        session.execute(
            delete(models.FundamentalSnapshot).where(
                models.FundamentalSnapshot.instrument_id == key
            )
        )
        session.commit()
