...
@pytest.mark.parametrize("path", [
    "/api/v1/terminal/bootstrap", "/api/v1/environment", "/api/v1/portfolios",
    "/api/v1/operations/portfolio", "/api/v1/research", "/api/v1/fundamentals/AAPL",
    "/api/v1/desks/quant", "/api/v1/providers", "/api/v1/terminal/runs",
    "/api/portfolio", "/api/risk", "/api/environment",
])
...
def test_hosted_admin_setup_is_not_public_registration(monkeypatch):
    ...
    with TestClient(app) as anonymous:
        response = anonymous.post("/api/v1/auth/setup", json={
            "email": "blocked@example.test", "password": secrets.token_urlsafe(24),
        })
...
def test_sign_in_logout_revocation_expiry_and_inactive_user():
    ...
    with TestClient(app) as browser:
        ...
        assert browser.post("/api/v1/auth/login", json={"email": email, "password": "wrong"}).status_code == 401
        ...
        with SessionLocal() as session:
            row = session.scalar(select(models.UserSession).where(models.UserSession.session_hash == token_digest(token)))
        ...
        assert browser.get("/api/v1/quotes").status_code == 401
        with SessionLocal() as session:
            row = session.scalar(select(models.UserSession).where(models.UserSession.user_id == user_id))
            row.revoked_at = None
        ...
        with SessionLocal() as session:
            ...
            session.commit()
        assert browser.get("/api/v1/quotes").status_code == 401
        assert browser.post("/api/v1/auth/login", json={"email": email, "password": password}).status_code == 401
