...
from test_portfolio_operations import drop
...
def test_paper_snapshot_scopes_precedence_missing_fx_and_fill_approval(accounting_session):
    ...
    agent = models.LocalAgent(name="Scoped broker", token_hash=hashlib.sha256(b"test").hexdigest(), scopes=["files:upload"], status={})
    session.add(agent)
    session.flush()
    now = datetime.now(timezone.utc)
    payload = SnapshotRequest(account="DU123456", currency="SGD", as_of=now, nav="71000", cash=[{"currency":"USD", "amount":"100"}],
        positions=[{"symbol":"AAPL","currency":"USD","quantity":"41","average_cost":"210","market_price":"220"}],
        fills=[{"execution_id":"E1","symbol":"AAPL","currency":"USD","side":"BOT","quantity":"1","price":"220","time":now,"commission":"1","commission_currency":"USD"}])
    ...
    assert account_view(session, PortfolioValuationService(session).latest())["account_source"] == "INTERNAL LEDGER"


def test_broker_rejects_live_account_and_future_timestamp(accounting_session):
    session = accounting_session
    agent = models.LocalAgent(name="test", token_hash="different", scopes=["broker:read-sync", "portfolio:" + profile_for(session).portfolio_id], status={})
    session.add(agent)
    session.flush()
    with pytest.raises(HTTPException) as exc:
        receive_snapshot(SnapshotRequest(account="U12345", currency="SGD", as_of=datetime.now(timezone.utc), nav="1"), agent, session)
...
def test_agent_queue_retry_archive_and_pause(tmp_path):
    ...
    def transport(request):
        ...
        if request.url.path == "/agent/v1/files" and request.method == "POST":
            return httpx.Response(200, json={"id":"file1","hash":hashlib.sha256(data).hexdigest(),"state":"UPLOAD_FAILED" if failed[0] else "MAPPING_REQUIRED"})
...
def test_broker_reader_is_paper_and_readonly(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "ib_async", SimpleNamespace(IB=FakeIB, StartupFetch=Fetch, ExecutionFilter=lambda **kwargs:kwargs))
    monkeypatch.setitem(sys.modules, "agent", load_agent())
    spec = importlib.util.spec_from_file_location("broker_reader", Path(__file__).parents[2] / "local-agent" / "broker_reader.py")
    reader = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(reader)
    args = SimpleNamespace(host="127.0.0.1",port=7497,account="DU12345",currency="SGD",client_id=3110)
