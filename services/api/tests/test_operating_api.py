...
def test_portfolio_operating_api_and_export():
    ...
    assert {"Summary", "Positions", "NAV History", "Trade Monitor", "Metadata"} <= set(book.sheetnames)
    assert client.post("/api/v1/operations/reset-demo", json={"confirmation": "no"}).status_code == 422
    assert client.post("/api/v1/operations/reconciliation").json()["state"] == "BROKER_NOT_CONNECTED"


def test_agent_pairing_scopes_single_use_revocation():
    pairing = client.post("/api/v1/data-drop/agents/pair").json()
    claim = client.post("/agent/v1/pair", json={"code": pairing["code"], "name": "Test agent " + str(uuid.uuid4())})
    assert claim.status_code == 200
    payload = claim.json()
    headers = {"Authorization": "Bearer " + payload["token"]}
    assert payload["scopes"] == ["files:upload", "files:status", "agent:heartbeat"]
    assert client.post("/agent/v1/pair", json={"code": pairing["code"], "name": "Replay"}).status_code in {403, 409}
    assert client.post("/agent/v1/heartbeat", json={"queued": 1}, headers=headers).status_code == 200
    assert client.get("/agent/v1/files").status_code == 401
    upload = client.post("/agent/v1/files", files={"file": (f"agent-{uuid.uuid4()}.csv", "date,close\n2026-09-04,100\n", "text/csv")}, headers=headers)
    ...
    assert client.post("/api/v1/data-drop/agents/" + payload["agent_id"] + "/revoke").status_code == 200
    assert client.get("/agent/v1/files", headers=headers).status_code == 401


def test_agent_endpoint_and_path_boundaries(tmp_path):
    spec = importlib.util.spec_from_file_location("local_data_agent", Path(__file__).parents[2] / "local-agent" / "agent.py")
...
def test_candidate_requires_pinned_version_and_blocks_unverified_promotion():
    ...
    created = client.post("/api/v1/desks/candidates", json={
    ...
    denied = client.post("/api/v1/desks/candidates/" + data["id"] + "/review", json={
        "state":"PAPER TESTING", "note":"Review without verified paper history",
    })
    assert denied.status_code == 422
    reviewed = client.post("/api/v1/desks/candidates/" + data["id"] + "/review", json={
        "state":"REJECTED", "note":"Insufficient evidence after review",
    })
    assert reviewed.status_code == 200 and reviewed.json()["state"] == "REJECTED"
    missing = client.post("/api/v1/desks/candidates", json={
        "name":"Invalid candidate", "hypothesis":"Missing immutable dataset history",
