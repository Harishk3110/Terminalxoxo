        ...
        for symbol in ANCHORS:
            session.add(
                models.Instrument(
                    symbol=symbol,
                    name=symbol,
                    country="Singapore" if symbol == "D05" else "United States",
                    currency="SGD" if symbol in {"D05", "USD.SGD"} else "USD",
                    asset_class="ETF" if symbol == "SPY" else "Equity",
                    security_type="Common Stock",
                    sector="Financials" if symbol == "D05" else "Technology",
                )
            )
        session.flush()
        ensure_main(session)
...
def test_average_cost_partial_sale_and_realised_pnl() -> None:
    ...
    state.apply(
        entry("BUY", instrument_id="A", quantity=D("10"), price=D("120"), commission=D("2"))
    )
...
def test_native_cash_fx_conversion_and_income() -> None:
    ...
    state.apply(
        entry(
            "FX_CONVERSION", amount=D("1300"), metadata={"to_currency": "USD", "to_amount": "1000"}
        )
    )
    state.apply(
        entry(
...
def test_split_and_spinoff_preserve_book_cost() -> None:
    ...
    state.apply(
        entry(
            ...
            instrument_id="A",
            quantity=D("5"),
            metadata={"child_instrument_id": "B", "cost_allocation": ".25"},
        )
    )
...
def test_transfers_and_correction_are_explicit() -> None:
    ...
    state.apply(
        entry(
            "OTHER_ADJUSTMENT",
            amount=D("10"),
            metadata={"reason": "Correction", "direction": "CREDIT"},
        )
    )
...
def test_nav_full_balance_sheet_and_flow_adjustment() -> None:
    result = nav_total(
        ...
        [D("51000"), D("-500")],
        {
            "accrued_income": D("100"),
            "receivables": D("200"),
            "payables": D("50"),
            "accrued_fees": D("20"),
            "other_liabilities": D("30"),
        },
    )
...
def test_seed_nav_and_independent_reconciliation(accounting_session: Session) -> None:
    ...
    assert result["performance"]["twr"] == pytest.approx(
        float(D(result["portfolio"]["nav"]) / D("100000") - 1), abs=1e-7
    )
...
def test_stale_file_beats_newer_demo_and_missing_is_not_zero(accounting_session: Session) -> None:
    ...
    accounting_session.add(
        models.MarketObservation(
            ...
            fields={"filename": "AAPL_2026-08-20.csv"},
        )
    )
...
def test_missing_selected_price_invalidates_complete_nav(accounting_session: Session) -> None:
    ...
    accounting_session.add(
        models.SourcePrecedenceRule(
            ...
            preferred_source="Unavailable approved provider",
            stale_after_hours=72,
            reason="Explicit provider selection test",
        )
    )
    accounting_session.commit()
