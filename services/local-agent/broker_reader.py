...
import httpx
from ib_async import IB, StartupFetch, ExecutionFilter
...
async def read_snapshot(args):
    if args.host not in {"localhost", "127.0.0.1", "::1"} or args.port not in {7497, 4002} or not args.account.startswith("DU"):
        raise ValueError("Only loopback TWS/Gateway paper ports and an explicit DU paper account are allowed")
    ib = IB()
    try:
        fields = StartupFetch.POSITIONS | StartupFetch.ACCOUNT_UPDATES | StartupFetch.EXECUTIONS
        await ib.connectAsync(args.host, args.port, clientId=args.client_id, timeout=10, readonly=True, account=args.account, fetchFields=fields)
        ...
        navs = [r for r in summary if r.tag == "NetLiquidation" and r.currency in {args.currency, "BASE"}]
        ...
        fx = {v.currency: v.value for v in values if v.tag == "ExchangeRate" and len(v.currency) == 3 and finite(v.value, True)}
        fx[args.currency] = "1"
        cash = [{"currency": r.currency, "amount": r.value} for r in values if r.tag == "CashBalance" and r.currency != "BASE" and len(r.currency) == 3 and finite(r.value) is not None]
        positions = []
        for row in ib.portfolio(args.account):
            ...
            positions.append({"symbol": contract.symbol if contract.secType == "STK" else contract.localSymbol, "currency": contract.currency, "quantity": str(row.position), "average_cost": average, "multiplier": multiplier, "market_price": finite(row.marketPrice, True)})
        fills = []
        for fill in await ib.reqExecutionsAsync(ExecutionFilter(acctCode=args.account)):
            ...
            fills.append({"execution_id": execution.execId, "symbol": contract.symbol if contract.secType == "STK" else contract.localSymbol, "currency": contract.currency, "contract_type": contract.secType, "side": execution.side, "quantity": str(execution.shares), "price": str(execution.price), "time": execution.time.astimezone(timezone.utc).isoformat(), "commission": finite(commission.commission), "commission_currency": commission.currency or None})
        return {"account": args.account, "currency": args.currency, "as_of": datetime.now(timezone.utc).isoformat(), "nav": navs[0].value, "cash": cash, "positions": positions, "fills": fills, "fx": fx}
    finally:
        ib.disconnect()
...
async def run(args):
    ...
    async with httpx.AsyncClient(base_url=url, headers={"Authorization": "Bearer " + token}, timeout=30, follow_redirects=False) as client:
        while True:
            try:
...
def main():
    ...
    parser.add_argument("--currency", required=True, help="Verified base currency of the paper account")
