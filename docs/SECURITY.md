...
- CORS allowlist for terminal origins.
...
- Every investment API requires authentication, including local demo and legacy aliases.
- Security tests verify removed visitor routes, sessions, revocation, expiry and inactive users.
...
- Sign-out revokes the current persisted session; account-wide session management is not yet exposed.
