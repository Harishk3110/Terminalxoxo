...
- 71169ba: full backend 32 failed two seed-help tests / 1,881 passed / 123
  ...
  synchronized pool first_connect event; failed connections close explicitly.
  Four negative controls fail before the correction, then 27 focused and 208
  affected tests pass. Three strict files pass; whole strict 48 remains FAIL at
  660 distinct diagnostics / 67 files / 313 sources / nine distributions.
  Browser 08 passes both workflows in 55.135s without retries/errors/skips;
  all four screenshots inspected and no API error/500/lock matches. Whole Ruff
  and formatting, secret scan (655 files / zero findings), no-execution scan pass.
