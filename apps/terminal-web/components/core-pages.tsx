...
export function PerformancePage() {
  ...
  return (
    <>
      <PageTitle code="PERF" title="Performance / Recorded Ledger">
      ...
      <Kpis
        ...
        asOf={d?.as_of}
        items={[
          { label: "Time-weighted return", value: pct(d?.performance.twr) },
          { label: "CAGR", value: pct(d?.performance.cagr) },
          { label: "Volatility", value: pct(d?.performance.volatility) },
          { label: "Sharpe", value: number(d?.performance.sharpe) },
          { label: "Sortino", value: number(d?.performance.sortino) },
          {
            label: "Max drawdown",
            value: pct(d?.performance.max_drawdown),
            className: "negative",
          },
        ]}
      />
      <div className="page-grid analytics-grid">
        ...
        >
          <LineChart
            label="Performance equity curve"
            rows={records(d?.curve ?? [])}
            keys={[
              { key: "equity", name: "KnK reference" },
              { key: "benchmark", name: "SPY", color: COLORS.blue },
            ]}
          />
        </Panel>
        <Panel
        ...
        >
          <DataTable
            id="monthly-returns"
            rows={records(d?.monthly ?? [])}
            columns={[
              { key: "month", label: "Month" },
              { key: "return", label: "Return", numeric: true, percent: true },
            ]}
          />
        </Panel>
        <Panel
          title="Drawdown / since inception"
          source={d?.source}
          asOf={d?.as_of}
          quality={d?.quality}
        >
        ...
        >
          <DataTable
            id="daily-returns"
            rows={records([...(d?.curve ?? [])].reverse())}
            columns={[
              { key: "date", label: "Date" },
              { key: "equity", label: "NAV (SGD)", numeric: true },
              { key: "return", label: "Return", numeric: true, percent: true },
            ]}
          />
        </Panel>
      </div>
      <p className="source-note">{d?.warnings[0]}</p>
    </>
  );
}
